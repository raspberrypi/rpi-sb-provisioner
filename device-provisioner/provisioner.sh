...
while getopts ${OPTSTRING} opt; do
    case ${opt} in
        ...
        k)
            ...
            ;;
        p)
            RPI_DEVICE_GENERATION=$(check_pidevice_generation "${OPTARG}")
            ;;
        s)
...
cp /usr/share/misc/fastboot-gadget.img "${RPI_SB_WORKDIR}"/boot.img

cp /usr/share/misc/boot_ramdisk_config.txt "${RPI_SB_WORKDIR}"/config.txt
...
#if ${RPI_DEVICE_GENERATION} != ${TARGET_PI_GENERATION}; then
#    echo "Raspberry Pi Generation mismatch. Expected a Raspberry Pi ${TARGET_PI_GENERATION} class device, you supplied ${RPI_DEVICE_GENERATION}"
#    exit 1
#fi
...
if [[ -z $(check_file_is_expected "${RPI_SB_WORKDIR}"/bootfs-temporary.img "img") ||
    ...
    cp /usr/share/misc/cryptroot_initramfs "${TMP_DIR}"/rpi-boot-img-mount/initramfs8
    cp /usr/share/misc/cryptroot_initramfs "${TMP_DIR}"/rpi-boot-img-mount/initramfs_2712
    ...
    cp /usr/share/misc/boot_ramdisk_config.txt "${TMP_DIR}"/config.txt

    make-boot-image -b "pi${RPI_DEVICE_GENERATION}" -d "${TMP_DIR}"/rpi-boot-img-mount -o "${TMP_DIR}"/boot.img
