...
WARNING: Traditional `.img` files *must* be uncompressed.

NOTE: When selecting an IDP artefact through the WebUI, the device family, storage type, and cipher fields are locked to the values defined in the artefact's metadata and cannot be overridden.
