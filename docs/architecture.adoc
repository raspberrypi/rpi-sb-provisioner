...
WARNING: Provisioning is triggered automatically when a device is connected.
...
|Raw `.img`
|Raw `.img`
|JSON descriptor + sparse images

|*Partition creation*
|Host-side
|Host-side
|Host-side
|Device-side (fastbootd)

|*Use case*
|Production devices
|Encryption without boot restrictions
|Development, testing
|Pre-built images from `rpi-image-gen`
|===
