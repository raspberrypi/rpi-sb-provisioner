...
The metadata is inserted into the `keywriter` log for the device, and also as a serial-number named JSON file under the `metadata` subdirectory of the device provisioning logs.
...
** If your device has not been recorded as having been through the *keywriter* phase, move to the *keywriter* phase
** If the device has been recorded as having been through the *keywriter* phase, move to the *provisioner* phase.
...
Logs are stored on a per-device, per-stage basis, where logs for a given device are stored at `/var/log/rpi-sb-provisioner/<serial>/<phase>.log`.

For example, to observe the progress of an individual device through a stage, you could use `tail`:
